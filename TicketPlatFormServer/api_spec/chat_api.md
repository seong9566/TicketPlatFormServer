...
**참고사항**:
- **정렬 순서**: 응답은 **최근 메시지부터 내림차순** (newest → oldest)으로 반환됩니다.
- **UI 표시**: 채팅 UI에서 "과거→현재" 순서로 표시하려면 클라이언트에서 배열을 역정렬해야 합니다.
- **무한 스크롤**:
  - 첫 조회: `lastMessageId` 없이 호출하여 최신 메시지 N개 조회
  - 이전 메시지 조회: 응답의 마지막(가장 오래된) 메시지 ID를 `lastMessageId`로 전달
  - Cursor 기반 페이지네이션으로 중복 없이 과거 메시지 탐색 가능
