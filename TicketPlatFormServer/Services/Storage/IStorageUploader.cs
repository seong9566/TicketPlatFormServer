...
public interface IStorageUploader
{
    ...
    /// </summary>
    Task<string> GetSignedUrlAsync(string objectKey, int expirySec, CancellationToken ct = default);

    /// <summary>
    /// 배치 signed URL 생성 (N+1 문제 해결)
    /// </summary>
    Task<Dictionary<string, string>> GetSignedUrlsBatchAsync(IEnumerable<string> objectKeys, int expirySec, CancellationToken ct = default);
